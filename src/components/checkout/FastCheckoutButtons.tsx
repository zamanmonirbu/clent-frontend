...
interface FastCheckoutButtonsProps {
  onSuccess: () => void;
}

export function FastCheckoutButtons({ onSuccess }: FastCheckoutButtonsProps) {
  const { items } = useCart();
  ...
  const handleCheckout = async () => {
    try {
      ...
      console.log(items)
      await createCheckoutSession(items); // Calls the function to initiate payment
      onSuccess(); // Callback for successful checkout
    } catch (error) {
  ...
  return (
    <div className="space-y-3">
      ...
      <Button
        ...
        onClick={() => {
          // You can add Apple Pay-specific logic here
          onSuccess();
        }}
      >
      ...
      <Button
        ...
        onClick={handleCheckout} // Trigger Stripe checkout
      >
        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
